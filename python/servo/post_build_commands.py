...
class PostBuildCommands(CommandBase):
    ...
    @CommandArgument(
        ...
        action="store_true",
        default=False,
        help="Android only: build a signed release .apk (Gradle's `assembleRelease`) instead of the "
        "default debug .apk. Requires APK_SIGNING_KEY_STORE_PATH/_STORE_PASS/_ALIAS/_PASS to already "
        "be set in the environment (read directly by "
        "support/android/apk/buildSrc/src/main/kotlin/Android.kt's getSigningKeyInfo, part of "
    ...
    def bundle(
        ...
        * Linux: by default, a single `play` binary. With --deb, a proper
          ...
          does not attempt).
        * Android (--android): a debug .apk by default, built via a real
          Gradle invocation (not a single-binary copy like the desktop
          platforms above) — see `_bundle_android`. `--content-dir` is
          copied into the APK's own assets instead of being packed/
          extracted at runtime. The game's own web app manifest
          (`manifest.webmanifest`/`manifest.json`/`site.webmanifest`, see
          `_read_web_manifest`) defaults the app label, locked orientation,
          and status bar color — each overridable via --android-app-name/
          --android-orientation/--android-theme-color. The launcher icon
          reuses the same --icon-png/auto-detected icon every other
          platform gets, not a separate manifest `icons`-array reading.
        ...
        # `servo_binary` is `None` for a "packaged" target (Android, OpenHarmony) --
        # command_base.py's own `binary_selection` deliberately nulls it out for those (see
        # that file's own comment: there's no single runnable binary to hand back). Only
        # matters here as a *default* output location when `--output` isn't given --
        # `_bundle_android` below derives everything it actually needs from
        # `self.target.triple()` instead, not from this.
        binary_dir = path.dirname(servo_binary) if servo_binary is not None else self.get_top_dir()
    ...
    def _bundle_android(
        ...
        if android_release and not os.environ.get("APK_SIGNING_KEY_STORE_PATH"):
            print(
                "--android-release requires APK_SIGNING_KEY_STORE_PATH (and "
                "_STORE_PASS/_ALIAS/_PASS) to already be set in the environment -- without "
                "them, Gradle's own signing config falls back to the debug key (see "
                "buildSrc/src/main/kotlin/Android.kt's getSigningKeyInfo), which defeats the "
                "point of asking for a release build at all. Refusing rather than silently "
