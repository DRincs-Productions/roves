...
impl ProtocolHandler for RovesProtocolHandler {
    ...
    fn load(
        ...
        if command == "save_file" {
            ...
            // future `save_file` hands back (see that method's own doc comment).
            let outcome_future = 'setup: {
                let Some(filename) = query.get("filename").cloned() else {
                    break 'setup Box::pin(std::future::ready(Err(
                        "Missing 'filename' parameter".to_owned(),
                    ))) as Pin<Box<dyn Future<Output = Result<(), String>> + Send>>;
                };
                let Some(data) = query.get("data").and_then(|data| base64_decode(data)) else {
                    break 'setup Box::pin(std::future::ready(Err(
